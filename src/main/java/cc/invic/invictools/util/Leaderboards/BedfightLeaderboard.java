package cc.invic.invictools.util.Leaderboards;
...
public class BedfightLeaderboard
{
      ...
      //  List<String> playerData = ;/*getStringList("data");*/
        for (String stringUuid : data.getConfigurationSection("data").getKeys(false))
        {
                    ...
                    && !Bukkit.getOfflinePlayer(UUID.fromString(stringUuid)).getName().equalsIgnoreCase("mayberry15"))
            {
                ...
                {
                    if (data.getInt("data." + stringUuid + ".Losses") + data.getInt("data." + stringUuid + ".Wins") >= cc.invic.invictools.util.Leaderboards.leaderboard.gamesBeforeLeaderboard)
                    {
        ...
        leaderboard = temp;
        makeLeaderboardText(cc.invic.invictools.util.Leaderboards.leaderboard.lbsize);
    }
    ...
    private void makeLeaderboardText(int size)
    {
        ...
        for (OfflinePlayer player : leaderboard.keySet())
        {
            ...
            if (i < size)
            {
                ...
                {
                    String s = ChatColor.translateAlternateColorCodes('&', " &b&l" + (i + 1) + ". &f" + playername + "&b - "+ new bedfightStatistics().presColor(((int) Math.round(leaderboard.get(player)))) + (int) Math.round(leaderboard.get(player))+"山 ");
                    formattedLeaderboard.add(s);
                }
                else
                {
    ...
    public void giveBFPosition(OfflinePlayer finded, Player p)
    {
        if (leaderboard.get(finded) == null)
        {
            ...
            p.sendMessage(ChatColor.AQUA + "" + ChatColor.BOLD + finded.getName() + ChatColor.WHITE + " is number" + ChatColor.AQUA + " " + ChatColor.BOLD + Placement.get(finded) + ChatColor.WHITE + " out of" + ChatColor.AQUA + " " + ChatColor.BOLD + leaderboardTotal + ChatColor.WHITE + " with" + ChatColor.AQUA + " " + new bedfightStatistics().presColor(((int) Math.round(leaderboard.get(finded)))) + (int) Math.round(leaderboard.get(finded)) + ChatColor.WHITE +" "+new BedfightLeaderboardHologram().modifySort(Sort).toLowerCase(Locale.ROOT)+"山");
        else
