...
public class Commands implements CommandExecutor, TabExecutor {
  ...
  public List<String> onTabComplete(
    ...
    if (args.length == 1) {
    ...
    } else if (args.length == 2 && args[0].equalsIgnoreCase("ProximityElytra")
        || args.length == 2 && args[0].equalsIgnoreCase("ProximityElytraSingle")) {
      ...
      tabComplete.add(
          "<all or one> <delay randomness> <delay min> <effect time> <effect level> <effect type> <player>");
      ...
      tabComplete.add("SingleArena");
        ...
        && args[1].equalsIgnoreCase("EveryArena")) {
      tabComplete.add("<team size>");
      tabComplete.add("2");
    } else if (args.length == 3
        && args[0].equalsIgnoreCase("SetTeamSize")
        && args[1].equalsIgnoreCase("SingleArena")) {
      ...
      tabComplete.add("2");
        ...
        && args[0].equalsIgnoreCase("SetTeamSize")
        && args[2].equalsIgnoreCase("SingleTeam")) {
    ...
    } else if (args.length == 2 && args[0].equalsIgnoreCase("CloseTeammateEffectSingle")
        || (args.length == 2 && args[0].equalsIgnoreCase("CloseTeammateEffect"))) {
      tabComplete.add("<effect>");
      tabComplete.add("FAST_DIGGING");
    } else if (args.length == 3 && args[0].equalsIgnoreCase("CloseTeammateEffectSingle")
        || (args.length == 3 && args[0].equalsIgnoreCase("CloseTeammateEffect"))) {
      tabComplete.add("<player>");
      for (Player player : Bukkit.getOnlinePlayers()) {
        tabComplete.add(player.getName());
      }
      ...
      tabComplete.add(
          "<every player, specified player, reset for all players, decrease over time, randomize>");
    } else if (args.length == 3
            && args[0].equalsIgnoreCase("sethealth")
            && args[1].equalsIgnoreCase("all")
        || args.length == 3
            && args[0].equalsIgnoreCase("sethealth")
            && args[1].equalsIgnoreCase("one")) {
      tabComplete.add("1");
      tabComplete.add("<health value>");
    } else if (args.length == 3
            && args[0].equalsIgnoreCase("sethealth")
            && args[1].equalsIgnoreCase("randomall")
        || args[1].equalsIgnoreCase("decrease")) {
      tabComplete.add("40");
      ...
      tabComplete.add("<interval in seconds>");
        ...
        || args.length == 2 && args[0].equalsIgnoreCase("tntsingle")) {
      ...
      tabComplete.add("Config");
        ...
        && args[0].equalsIgnoreCase("itemrain")
        && args[1].equalsIgnoreCase("LootTable")) {
      ...
      tabComplete.add(
          "<item> <amount> <interval> <normal/random/specificitem/specificlb> <name/all>");
      ...
      tabComplete.add("INCREASE_DAMAGE 1 100 Invictable");
        ...
        || args.length == 2 && args[0].equalsIgnoreCase("randomeffectsingle")) {
    ...
    if (sender.hasPermission("invic.invictools")) return tabComplete;
    else return null;
  }
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    if (args.length >= 2 && args[0].equalsIgnoreCase("BedBreak")) {
      ...
      new TeamSelection()
          .openInventory(
              (Player) sender, BedwarsAPI.getInstance().getGameOfPlayer((Player) sender), true);
      sender.sendMessage("Opened team selection");
    } else if (args.length == 2 && args[0].equalsIgnoreCase("panel")) {
      if (!args[1].equals("reload")) new panels().openInventory(args[1], (Player) sender);
      else {
      ...
      new leaderboardHologram().createLeaderboard();
        ...
        && args[0].equalsIgnoreCase("leaderboard")
        && args[1].equalsIgnoreCase("position")) {
      ...
      new leaderboard()
          .givePosition(
              Bukkit.getOfflinePlayer(UUID.fromString(sender.getName())), (Player) sender);
      ...
      if (currentgame.equalsIgnoreCase("none")) {
        ...
        new BukkitRunnable() {
          @Override
          public void run() {
            if (!BedwarsAPI.getInstance()
                .getGameByName(currentgame)
                .getStatus()
                .equals(GameStatus.WAITING))
              if (player
                  .getWorld()
                  .equals(BedwarsAPI.getInstance().getGameByName(currentgame).getGameWorld()))
      ...
      Player player = (Player) sender;
      Plugin plugin = Bukkit.getServer().getPluginManager().getPlugin("Invictools");
      //   File pFile = new File(plugin.getDataFolder().getPath() + "\\PlayerData" + File.separator
      ...
      try {
        ...
        && args[0].equalsIgnoreCase("FinalKillTest")
        && args[2].equalsIgnoreCase("cp")) {
      Player player = (Player) sender;
      ...
      player
          ...
          .sendMessage(
              ...
              new TextComponent(ChatColor.BOLD + "Hold crouch to cancel"));
      BukkitRunnable runnable2 =
          new BukkitRunnable() {
            ...
            public void run() {
              i++;
              if ((player.getLocation().getWorld() == oldloc.getWorld() && i == cancelAt)
                  || player.isSneaking()) {
                player.teleport(oldloc);
                this.cancel();
              }

              if (i == cancelAt) this.cancel();
            }
          };
      runnable2.runTaskTimer(
          Objects.requireNonNull(Bukkit.getServer().getPluginManager().getPlugin("Invictools")),
          10,
          1);
    } else if (args.length >= 2
        && args[0].equalsIgnoreCase("NormalKillPreview")
        && args[2].equalsIgnoreCase("cp")) {
      Player player = (Player) sender;
      Location oldloc = player.getLocation();
      ...
      runnable2.runTaskTimer(
        ...
        && args[0].equalsIgnoreCase("BedBreakTest")
        && args[2].equalsIgnoreCase("cp")) {
      Player player = (Player) sender;
      Location oldloc = player.getLocation();
            ...
            public void run() {
              ...
              runnable2.runTaskTimer(
                  Objects.requireNonNull(
                      Bukkit.getServer().getPluginManager().getPlugin("Invictools")),
                  30,
                  1);
            }
          };
      ...
      for (Entity e : Bukkit.getWorld("bwlobby").getEntitiesByClass(ArmorStand.class)) {
        if (!e.hasMetadata("holo")) e.remove();
      }
    ...
    } else if (sender.hasPermission("invic.firestick")
        && args[0].equalsIgnoreCase("firestick")
        && FireStickEnabled) {
      if (Bukkit.getPlayer(sender.getName()).getWorld().getName().equals("bwlobby"))
        new FireStick((Player) sender);
    } else if (sender.hasPermission("invic.firestick")
        && args[0].equalsIgnoreCase("lobbydare")
        && FireStickEnabled) {
      ...
      sender.sendMessage(ChatColor.GOLD + "Lobby Dare " + ChatColor.RED + "is currently disabled.");
        ...
        && !FireStickEnabled) {
      sender.sendMessage(
          ChatColor.GOLD + "FireSticks " + ChatColor.RED + "are currently disabled.");
      ...
      if (args.length > 0 && args[0].equalsIgnoreCase("destroyholos")) {
        ...
        //  Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "minecraft:kill
        // @e[type=minecraft:armor_stand]");
        sender.sendMessage(
            ChatColor.AQUA + "All non scoreboard holographic displays have been deleted!");
      } else if (args.length >= 1 && args[0].equalsIgnoreCase("debug")) {
        sender.sendMessage(" ");
        sender.sendMessage(ChatColor.WHITE + "Status:");
        sender.sendMessage(
            ChatColor.YELLOW + "- Master Player: " + ChatColor.WHITE + MasterPlayer.getName());
        sender.sendMessage(
            ChatColor.YELLOW + "- Lucky Blocks Enabled: " + ChatColor.WHITE + LuckyBlocksEnabled);
        sender.sendMessage(
            ChatColor.YELLOW + "- FireStick Enabled: " + ChatColor.WHITE + FireStickEnabled);
        sender.sendMessage(ChatColor.YELLOW + "- Teammates Assignment: ");
        sender.sendMessage(ChatColor.YELLOW + "- Lobby: ");
        LobbyLogic.lobbyRegion.forEach(
            (attribute, value) -> sender.sendMessage(ChatColor.WHITE + attribute + " " + value));
        sender.sendMessage(ChatColor.YELLOW + "- Attributes: ");
        AbtributesOnDeath.ActiveAttributes.forEach(
            (attribute, value) ->
                sender.sendMessage(ChatColor.WHITE + attribute + " at value " + value));
        sender.sendMessage(ChatColor.YELLOW + "- ProximityElytra: ");
        ProximityElytra.forEach(
            (p, bool) ->
        ...
        DeathCounter.deaths.forEach(
            (p, value) ->
                sender.sendMessage(
                    ChatColor.WHITE + p.getName() + " has this many deaths: " + value));
        sender.sendMessage(ChatColor.YELLOW + "- Manhunt: ");
        sender.sendMessage(ChatColor.YELLOW + "Teams: ");
        ManhuntMain.ManhuntTeam.forEach(
            (p, team) -> sender.sendMessage(ChatColor.WHITE + p.getName() + " is on team " + team));
        sender.sendMessage(ChatColor.YELLOW + "Hunter items: ");
        ManhuntMain.HunterItems.forEach(
            (item) -> sender.sendMessage(ChatColor.WHITE + " hunter items: " + item));
        sender.sendMessage(ChatColor.YELLOW + "Hunter effects: ");
        ManhuntMain.HunterEffects.forEach(
            (item) -> sender.sendMessage(ChatColor.WHITE + " hunter effects: " + item));
        sender.sendMessage(ChatColor.YELLOW + "Hunted items: ");
        ManhuntMain.HuntedItems.forEach(
            (item) -> sender.sendMessage(ChatColor.WHITE + " hunted items: " + item));
        sender.sendMessage(ChatColor.YELLOW + "Hunted effects: ");
        ManhuntMain.HuntedEffects.forEach(
            (item) -> sender.sendMessage(ChatColor.WHITE + " hunted effects: " + item));
        ...
        sender.sendMessage(
            ChatColor.AQUA + "Manhunt team pre-assignments and data has been cleared");
      } else if (args.length >= 1 && args[0].equalsIgnoreCase("ResetAttributes")) {
        AbtributesOnDeath.resetAttributes();
        sender.sendMessage(
            ChatColor.AQUA + "All players active attributes modifiers have been reset");
        ...
        if (args[2].equalsIgnoreCase("player") || args[2].equalsIgnoreCase("p")) {
          new WardenSpawner()
              .ShriekerFromWorld(
                  Bukkit.getPlayer(args[3]).getWorld(), Boolean.parseBoolean(args[1]));
        } else
          new WardenSpawner()
              .ShriekerFromWorld(Bukkit.getWorld(args[3]), Boolean.parseBoolean(args[1]));
        ...
        new WorldBorder(
            ...
            false);
        sender.sendMessage(
            ChatColor.AQUA + "World border closing on x/z" + Integer.parseInt(args[4]));
        ...
        Player p = (Player) sender;
        if (args[1].equalsIgnoreCase("single")) noShop.put(Bukkit.getPlayer(args[2]), true);
        ...
        lore.add(
            ChatColor.translateAlternateColorCodes(
                '&', "&cStay near your teammates or you'll lose it!"));
        meta.setLore(lore);
        ...
        sender.sendMessage(
            ChatColor.AQUA + "Command went through but i didnt code this part yet so dont look");
      } else if (args.length == 1 && args[0].equalsIgnoreCase("printitemdata")
          || (args.length == 1 && args[0].equalsIgnoreCase("pid"))) {
        Player player = (Player) sender;
        ...
        sender.sendMessage(
            ChatColor.AQUA
                + "Logged the ItemStack in your offhand in the console and config : "
                + player.getInventory().getItem(40).getType());
        ...
        Bukkit.dispatchCommand(
            Bukkit.getConsoleSender(), "minecraft:kill @e[type=minecraft:armor_stand]");
        sender.sendMessage(
            ChatColor.AQUA + "Safe reloading... This command is pointless now you moron.");
        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "reload");
      } else if (args.length == 1 && args[0].equalsIgnoreCase("srestart")) {
        Bukkit.dispatchCommand(
            Bukkit.getConsoleSender(), "minecraft:kill @e[type=minecraft:armor_stand]");
        sender.sendMessage(
            ChatColor.AQUA + "Safe restarting... This command is pointless now you moron.");
        Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "restart");
      } else if (args.length == 1 && args[0].equalsIgnoreCase("ToggleFireStick")) {
        if (FireStickEnabled) FireStickEnabled = false;
        else FireStickEnabled = true;

        sender.sendMessage(
            ChatColor.AQUA + "Firestick Enabled is now " + ChatColor.YELLOW + FireStickEnabled);
        ...
        else VictoryDancePreview.VictoryPreviewEnabled = true;

        sender.sendMessage(
            ChatColor.AQUA
                + "Victory Dance Preview Enabled is now "
                + ChatColor.YELLOW
                + VictoryDancePreview.VictoryPreviewEnabled);
        ...
        sender.sendMessage(
            ChatColor.AQUA
                + "Raining items in the sky using LootTable "
                + ChatColor.YELLOW
                + args[2]);
        ...
        new AbtributesOnDeath()
              .AbtributesOnDeathAll(
                  (Player) sender,
                  args[2],
                  Double.parseDouble(args[3]),
                  Double.parseDouble(args[4]));
        else if (args[1].equalsIgnoreCase(
            "single")) // it DeathAttribute single Attribute interval basevalue player
        new AbtributesOnDeath()
              .AttributesOnDeathSingular(
                  getPlayer(args[5]),
                  args[2],
                  Double.parseDouble(args[3]),
                  Double.parseDouble(args[4]));
        ...
        if (args[1].equalsIgnoreCase("all")) {
          for (Player player : Bukkit.getOnlinePlayers()) {
            player.sendMessage(
                ChatColor.AQUA + "You can now haunt players after you are final killed");
            if (MasterPlayer.getWorld().equals(player.getWorld())) {
              Hauntable.put(player, true);
              World world = player.getWorld();

              new BukkitRunnable() // kills at max 1 minute after match ends or player dies and
                                   // leaves
              {
                @Override
                public void run() {
        ...
        if (BedwarsAPI.getInstance().isPlayerPlayingAnyGame(p)) {
          ...
          sender.sendMessage(
              ChatColor.AQUA + "Lucky Blocks enabled in world " + ChatColor.YELLOW + worldname);
          FileConfiguration config =
              new LobbyLogic()
                  .getMapConfiguration(BedwarsAPI.getInstance().getGameOfPlayer(p).getName());
          ...
          if (spawnerSize != 0) {
            ...
            for (int a = 0; a < spawnerSize; a++) {
              ...
              Location spawnerloc =
                  new Location(
                      p.getWorld(),
                      config.getDouble("LuckySpawners." + a + ".x"),
                      config.getDouble("LuckySpawners." + a + ".y"),
                      config.getDouble("LuckySpawners." + a + ".z"));
          ...
          new BukkitRunnable() {
            ...
            public void run() {
              if (!p.getWorld().getName().equals(worldname)) {
                sender.sendMessage(
                    ChatColor.AQUA
                        + "Lucky Blocks disabled in world "
                        + ChatColor.YELLOW
                        + worldname);
        ...
        if (args[5].equalsIgnoreCase("all")) {
            ...
            if (args[4].equalsIgnoreCase("specificlb")) {
              ...
              player.sendMessage(
                  (ChatColor.AQUA
                      ...
                      + item.getAmount()
                      + " "
                      + item.getType().toString().toLowerCase(Locale.ROOT)
                      + ChatColor.AQUA
                      + " every "
                      + ChatColor.YELLOW
                      + delay
          ...
          && args[0].equalsIgnoreCase(
              "KillEffects")) // it killeffects POTTYPE POTLEVEL DURATION PLAYERNAME
      {
        if (args[4].equals("all")) {
          for (Player player : Bukkit.getOnlinePlayers()) {
            killEffects.put(
                player, args[1] + "-" + args[2] + "-" + args[3]); // pot type, levles, duartion
          ...
          killEffects.put(
              Bukkit.getPlayer(args[4]),
              args[1] + "-" + args[2] + "-" + args[3]); // pot type, levles, duartion
          ...
          && args[0].equalsIgnoreCase("KillItems")) // it killitems itemtype amount normal player
      {
        if (args[4].equals("all")) {
          for (Player player : Bukkit.getOnlinePlayers()) {
          ...
          if (args[3].equalsIgnoreCase("normal")) {
            ...
            sender.sendMessage(
                ChatColor.AQUA
                    + Bukkit.getPlayer(args[4]).getName()
                    + item.getType()
                    + " after every kill");
            new BukkitRunnable() {
              @Override
          ...
          && args[0].equalsIgnoreCase("DeathItems")) // it deathitems itemtype amount normal player
      {
        if (args[4].equals("all")) {
          for (Player player : Bukkit.getOnlinePlayers()) {
            if (args[3].equalsIgnoreCase("normal")) {
        ...
        for (PotionEffectType p : PotionEffectType.values()) {
          if (e1.equalsIgnoreCase(p.getName())) check = true;
        }
        if (!check) e1 = "LUCK";
        check = false;

        for (PotionEffectType p : PotionEffectType.values()) {
          if (e2.equalsIgnoreCase(p.getName())) check = true;
        }
        if (!check) e2 = "LUCK";
        ...
        sender.sendMessage(
            ChatColor.AQUA
                + "GiveItem global cancel: "
                + ChatColor.YELLOW
                + giveItemRepeated.repeatedCancel);
        ...
        sender.sendMessage(
            ChatColor.AQUA
                + "Master player is currently "
                + ChatColor.YELLOW
                + config.getString("masterplayer"));
      } else if (args.length == 1 && args[0].equalsIgnoreCase("alwaysbridge")) {
        Player player = (Player) sender;
        sender.sendMessage(
            ChatColor.AQUA
                + "Always bridging active in world "
                + ChatColor.YELLOW
                + player.getWorld().getName());
        for (Player p : Bukkit.getOnlinePlayers()) {
          if (player.getWorld().equals(p.getWorld())) new AlwaysBridge(p);
        }
      } else if (args.length >= 1 && args[0].equalsIgnoreCase("alwaysbridgesingle")) {
        Player player = (Player) sender;
        player.sendMessage(
            ChatColor.AQUA + "Scaffold now active while you have wool! Hold crouch to fall.");
        ...
        Player player = (Player) sender;
        sender.sendMessage(ChatColor.AQUA + "giving effects when you're near your teammate");
        new CloseEffectSpecific(
            getPlayer(args[3]), player.getWorld().getName(), args[1], Integer.parseInt(args[2]));
        ...
        if (args.length == 1) {
          ...
          sender.sendMessage(
              ChatColor.AQUA + "range: " + ChatColor.YELLOW + ExplosionsListener.range);
          sender.sendMessage(
              ChatColor.AQUA + "x: " + ChatColor.YELLOW + ExplosionsListener.xzmultiplier);
          sender.sendMessage(
              ChatColor.AQUA + "y: " + ChatColor.YELLOW + ExplosionsListener.ymultiplier);
        ...
        sender.sendMessage(
            ChatColor.YELLOW + args[1] + ChatColor.AQUA + " set to " + ChatColor.YELLOW + args[2]);
      } else if (args.length == 4 && args[0].equalsIgnoreCase("giveitem")) {
        ...
        if (arena != null) {
          ...
          sender.sendMessage(
              ChatColor.AQUA
                  ...
                  + arena
                  + ChatColor.AQUA
                  + " has been loaded into the npc");
        } else sender.sendMessage(ChatColor.RED + "Arena name is not valid");
        ...
        sender.sendMessage(ChatColor.AQUA + "Forbidden Drops cleared");
          ...
          && args[0].equalsIgnoreCase(
              "SetHealth")) // sethealth all/one/random/decrease/reset, value, player/interval
      {
        if (args[1].equalsIgnoreCase("all")) {
          new CustomHealth(
              "all",
          ...
          new CustomHealth(
              "one", p, Integer.parseInt(args[2]), 1, ((Player) sender).getWorld().getName());
          sender.sendMessage(
              ChatColor.AQUA
                  + "Maximum Health is now set to "
                  + ChatColor.YELLOW
          ...
          sender.sendMessage(
              ChatColor.AQUA + "Maximum Health has been reset to " + ChatColor.YELLOW + "20");
          ...
          sender.sendMessage(
              ChatColor.AQUA
                  + "Maximum Health will randomize with a max of "
                  + ChatColor.YELLOW
                  + Integer.parseInt(args[2])
                  + ChatColor.AQUA
                  + " every "
        ...
        Plugin plugin = Bukkit.getServer().getPluginManager().getPlugin("Invictools");
        //  File pFile = new File(plugin.getDataFolder().getPath() + "\\Spawner" + File.separator +
        // config + ".yml");
        ...
        if (test != null) {
          ...
          sender.sendMessage(
              ChatColor.AQUA
                  + "Custom spawner active in world "
                  + ChatColor.YELLOW
          ...
          sender.sendMessage(
              ChatColor.RED
                  + "Requires a valid config.yml and valid spawner location in this world");
        ...
        sender.sendMessage(
            ChatColor.AQUA
                + "Applying random effect with intensity limit "
                + ChatColor.YELLOW
                + intensityLimit
                + ChatColor.AQUA
                + " in world "
        ...
        sender.sendMessage(
            ChatColor.AQUA
                + "You will now receive random potion effects every "
                + ChatColor.YELLOW
                + effecttime);
        ...
        if (blacklistedWorlds.contains(worldName)) {
          sender.sendMessage(
              ChatColor.RED + "Cannot be activated in world " + ChatColor.YELLOW + worldName);
        } else {
          ...
          sender.sendMessage(
              ChatColor.AQUA
                  + "Spawning tnt on players at an interval of "
                  + ChatColor.YELLOW
                  + interval
                  + ChatColor.AQUA
                  + " in world "
        ...
        if (blacklistedWorlds.contains(worldName)) {
          ...
          sender.sendMessage(
              ChatColor.AQUA
                  + "TnT will now spawn on top of you every "
                  + ChatColor.YELLOW
                  + interval
                  + ChatColor.AQUA
                  + " seconds ");
        ...
        if (args.length > 1 && args[1].equalsIgnoreCase("invictools")) {
          ...
          //  Bukkit.dispatchCommand(Bukkit.getConsoleSender(), "bweffects reload"); // executes
          // below disable and enable from other plugin
         ...
         */
        sender.sendMessage(
            ChatColor.AQUA + "refer to google docs for updated list. most commands tab complete");
      }
    } else {
  ...
  Commands(List<String> worlds, double sety, List<String> blacklist, List<String> games) {
    worldstemp = worlds;
    ytemp = sety;
